...
public class GlobalExceptionHandler {
    ...
    public Object handleException(Exception e) {
        ...
        return ApiRestResponse.error(OAExceptionEnum.SYSTEM_ERROR);
    }

    @ExceptionHandler(OAException.class)
    @ResponseBody
    public Object handleImoocMallException(OAException e) {
    ...
    private ApiRestResponse handleBindingResult(BindingResult result) {
        ...
        if (result.hasErrors()) {   //是否包含错误
            ...
            if (list.size() == 0) {
                return ApiRestResponse.error(OAExceptionEnum.REQUEST_PARAM_ERROR);
            }
        }
        return ApiRestResponse.error(OAExceptionEnum.REQUEST_PARAM_ERROR.getCode(), list.toString());
    }
}
